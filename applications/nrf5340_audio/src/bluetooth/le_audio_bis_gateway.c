...
static int adv_create(void)
{
	...
	ext_ad[0].data_len = strlen(CONFIG_BT_AUDIO_BROADCAST_NAME);
	ext_ad[0].type = BT_DATA_BROADCAST_NAME;
	ext_ad[0].data = CONFIG_BT_AUDIO_BROADCAST_NAME;
...
int le_audio_ext_adv_set(struct bt_le_ext_adv *ext_adv)
{
	...
	if (ret) {
		LOG_ERR("Failed to start broadcast source");
		return ret;
	}
