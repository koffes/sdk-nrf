	...
	while (1) {
		...
		case LE_AUDIO_EVT_NOT_STREAMING:
			...
			for (int i = 0; i < CONFIG_BT_ISO_MAX_BIG; i++) {
				for (int j = 0; j < CONFIG_BT_BAP_BROADCAST_SRC_SUBGROUP_COUNT;
				     j++) {
					for (int k = 0;
					     k < CONFIG_BT_BAP_BROADCAST_SRC_STREAM_COUNT; k++) {
						ret = k_work_cancel_delayable(
							&bis_work_items[i][j][k].work);
						if (ret != 0 && ret != -EINVAL) {
							LOG_ERR("Failed to cancel BIS work: %d",
								ret);
						}
					}
				}
...
static void stream_queue_sdu_ref_update(const struct zbus_channel *chan)
{
	...
	if (tx_sync_ts_us < curr_ts_us) {
		LOG_WRN("TX sync timestamp is in the past compared to current timestamp");
		time_to_next_send_us = 10000;
	} else {
